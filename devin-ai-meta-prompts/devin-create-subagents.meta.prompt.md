...
- Frontmatter: `permissions.deny: [write, edit, run]`
...
allowed-tools:
    ...
    - create_file        # si crea archivos nuevos
    - edit_file          # si modifica archivos existentes
    - str_replace_editor # si hace reemplazos de texto
    - get_errors         # si necesita verificar errores de compilación
...
- `model`: usar `claude-sonnet-4-5` por defecto.
...
- Read-only agents: siempre `permissions.deny: [write, edit, run]`.
